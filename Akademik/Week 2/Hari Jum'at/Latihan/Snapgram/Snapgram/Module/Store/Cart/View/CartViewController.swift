...
class CartViewController: BaseViewController {
    ...
    internal func addItemToCoreData(for action: ProductCoreData, indexPath: IndexPath) -> CoreDataResult {
        ...
        case .favorite(let entity):
            ...
            return CoreDataHelper.shared.addOrUpdateEntity(FavoriteProducts.self, for: .favorite(id: entity), productId: Int(item.productID), properties: properties)
        default: break
        }
        return .failed
    }
...
extension CartViewController: CartTableCellDelegate {
    func incrementQty(index: IndexPath) {
        if let item = dataSource.itemIdentifier(for: index) {
            weak let _ = CoreDataHelper.shared.updateItemQty(Cart.self, productID: item.productID)
    ...
    func decrementQty(index: IndexPath) {
        if let item = dataSource.itemIdentifier(for: index) {
            let state = CoreDataHelper.shared.updateItemQty(Cart.self, productID: item.productID, isIncrement: false)
            if state == .deleted {
    ...
    func isExist(index: IndexPath) -> Bool {
        guard !snapshot.itemIdentifiers.isEmpty else { return false }
        let item = snapshot.itemIdentifiers[index.item]
        let result = CoreDataHelper.shared.isEntityExist(FavoriteProducts.self, productId: Int(item.productID))
        return result
    }
}
